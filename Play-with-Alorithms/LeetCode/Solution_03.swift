...
extension Solution_03 {
    ...
    func reverseList_1(_ head: ListNode?) -> ListNode? {
        ...
        var p: ListNode? = head
        var n: ListNode? = p?.next
        var d: ListNode? = n?.next
    ...
    func printLinkList(_ p: ListNode?) {
        if p == nil {
            return
        }
        var n: ListNode? = p
...
extension Solution_03 {
    ...
    func solveSudoku(_ board: inout [[Character]]) {
        ...
        var squareSet = Array(repeating: Array(repeating: Array(repeating: false, count: 9), count: 9), count: 9)
        var blank: [[Int]] = []
        ...
        func deep(_ pos: Int) {
            ...
            for x in 1 ... 9 {
                ...
                if !rowSet[row][x - 1] && !columnSet[col][x - 1] && !squareSet[row / 3][col / 3][x - 1] {
                    board[row][col] = Character("\(x)")
                    rowSet[row][x - 1] = true
                    columnSet[col][x - 1] = true
                    squareSet[row / 3][col / 3][x - 1] = true
                    deep(pos + 1)
                    // 当填写下一个空格，发现 1 到 9 都填不了时，则回溯到当前位置, 重新填写
                    rowSet[row][x - 1] = false
                    columnSet[col][x - 1] = false
                    squareSet[row / 3][col / 3][x - 1] = false
                }
            }
        }
    ...
    func printSudoku(_ arr: [[Character]]) {
        // | 1 3 4 | 5 6 7 | 8 9 0 |
        // |-----------------------|
        print("|-----------------------|")
        for i in 0 ..< arr.count {
            for j in 0 ..< arr[i].count {
                let c = arr[i][j] == "." ? " " : "\(arr[i][j])"
                var e = "\(c) "
                if j == 0 {
                    e = "| " + e
                }
