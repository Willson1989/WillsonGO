...
struct BasicBinaryTree<K: Comparable & Equatable, V> {
    typealias Node = TNode<K, V>
    typealias SearchHandler = (_ node: Node?) -> Void
    ...
    // 返回以 node 为根节点的树中 val 比 key值小的最大的元素的结点
    ...
    // 返回以 node 为根节点的树中 val 比 key值大的最小的元素的结点
